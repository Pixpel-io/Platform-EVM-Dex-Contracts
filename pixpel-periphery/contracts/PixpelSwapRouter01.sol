...
contract PixpelSwapRouter01 is IPixpelSwapRouter01 {
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'PixpelSwapV2Router: INSUFFICIENT_A_AMOUNT');
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IPixpelSwapPair(PixpelSwapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, PixpelSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'PixpelSwapRouter: INVALID_PATH');
        amounts = PixpelSwapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PixpelSwapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, PixpelSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, address(this));
        IWETH(WETH).withdraw(amounts[amounts.length - 1]);
        TransferHelper.safeTransferETH(to, amounts[amounts.length - 1]);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return PixpelSwapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
