...
describe('PixpelSwapERC20', () => {
  ...
  it('should get the deploying wallet address', async () => {
    console.log('Wallet Address:', wallet.address)
    expect(wallet.address).to.be.properAddress
  })

  it('should assign total supply to wallet', async () => {
    expect(await token.balanceOf(wallet.address)).to.equal(TOTAL_SUPPLY)
  })

  it('name, symbol, decimals, totalSupply, balanceOf, DOMAIN_SEPARATOR, PERMIT_TYPEHASH', async () => {
    const name = await token.name()
    expect(name).to.eq('Pixpel TestToken')
    expect(await token.symbol()).to.eq('PIXP')
